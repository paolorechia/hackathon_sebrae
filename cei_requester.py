...
def get_ceis_request(cnpj):
    response = requests.get(
             'http://www.transparencia.gov.br/api-de-dados/ceis?codigoSancionado={}&pagina=1'\
                .format(cnpj),
            headers={'Accept': '*/*'}
        )
...
if __name__ == "__main__":
    ...
    for cnpj in cnpj_list:
        print(cnpj)
        output_filename = cnpj + '_cei_response.json'
        if output_filename not in downloaded:
            try:
                response_json, status_code = get_ceis_request(cnpj)
                to_save_dict = {
            ...
            print('Skipping request for... {}'.format(cnpj))
